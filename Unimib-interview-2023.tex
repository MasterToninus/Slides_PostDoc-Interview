...
\begin{frame}\frametitle{The $L_\infty$-algebra of observables}
	\begin{itemize}
		\item $(M,\omega)$ $k$-plectic manifold
	\end{itemize}

	\[
		L_\infty(M,\omega) = \Omega_{\Ham}^{k-1}(M,\omega) \oplus \Omega^{\leq k-2}(M)
	\]

	The \emph{higher brackets} $\{l_i\}_{i\geq0}$ are given by
	\[
		l_i:\underbrace{L_\infty(M,\omega)\times\cdots\times L_\infty(M,\omega)}_{i \text{ times}} \to L_\infty(M,\omega)
	\]
	where
	\begin{align*}
		l_1(\alpha)		&=	\d\omega						\\
		l_2(\alpha,\beta)	&=	\iota_{X_\beta}\iota_{X_\alpha}\omega\hspace{1cm}\text{($X_\alpha=0$ if $\deg\alpha\leq n-1$)}			\\
		l_3(\alpha,\beta,\gamma)&=	\iota_{X_\gamma}\iota_{X_\beta}\iota_{X_\alpha}\omega	\\[-3pt]
					&\hspace{6pt}\vdots
	\end{align*}
\end{frame}
...
\begin{frame}\frametitle{\'Sniatycki--Weinstein reduction}
	...
	\begin{definition}[momentum ideal]
